...
def test_hist_cli(tmpdir):
    ...
    result = runner.invoke(
        hist, ["tests/data/source1.tif", "tests/data/reference1.tif", output]
    )
...
def test_hist_cli2(tmpdir):
    ...
    runner = CliRunner()
    result = runner.invoke(
        hist, ["tests/data/source2.tif", "tests/data/reference2.tif", output]
    )
    assert result.exit_code == 0
    with rasterio.open(output) as out:
...
def test_hist_cli_plot(tmpdir):
    ...
    result = runner.invoke(
        hist,
        ["-v", "--plot", "tests/data/source2.tif", "tests/data/reference2.tif", output],
    )
    assert result.exit_code == 0
    with rasterio.open(output) as out:
        assert out.count == 4  # RGBA
...
def test_partial(tmpdir):
    ...
    result = runner.invoke(
        hist,
        ["-m", "0.5", "tests/data/source1.tif", "tests/data/reference1.tif", output],
    )
    assert result.exit_code == 0
    with (
        rasterio.open(output) as match,
